...
def SingleGraph():
    ...
    #test to see if user input is legitimate directory
    dirValue = 0
    while dirValue == 0:
        print("Paste Directory Below")
        directory = input()
        if os.path.isdir(directory) == 1:
            dirValue = 1
        else:
            print(dir + " Is Not A Workind Directoy")
            print("Press Enter")
            input()
            clear = lambda: print('\n' * 100)
            clear()
...
while end == 0:
    print("type 1 To Graph Entire Contents of Directory, 2 to graph single folder")
    response = input()
    if response == '1':
        FullDirectory()
